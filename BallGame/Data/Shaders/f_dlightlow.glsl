...
void main() {
	...
	float shadow = 0.0;
	if ( mapNum < 0.5 )
		shadow += shadow2DProj(shadowMap[0], shadowCoord + vec4(0, 0, -0.0005, 0.0)).w;
	else if(mapNum<1.5)
		shadow += shadow2DProj(shadowMap[1], shadowCoord + vec4(0, 0, -0.001, 0.0)).w;
	else if(mapNum<2.5)
		shadow += shadow2DProj(shadowMap[2], shadowCoord + vec4(0, 0, -0.0015, 0.0)).w;
	else
		shadow += shadow2DProj(shadowMap[3], shadowCoord + vec4(0, 0, -0.002, 0.0)).w;
